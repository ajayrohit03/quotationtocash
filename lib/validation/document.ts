...
export const documentUpdateSchema = appearanceUpdateSchema.extend({
  ...
  // The document's own settlement currency — an ISO 4217-shaped code,
  // not validated against a fixed list (the GST/tax engine doesn't care
  // what this value is; only whether it equals "INR" — see
  // docs/foreign-currency-invoicing-design.md).
  currency: z.string().trim().min(3).max(3).optional(),
  // Display-only INR-equivalent conversion rate; see
  // appearanceUpdateSchema's `showInrEquivalent`.
  inrExchangeRate: z.number().positive().nullable().optional(),
  lutDeclarationText: z.string().trim().max(2000).nullable().optional(),
